package org.algorithms.hackerrank.medium;
...
public class ClimbingLeaderboard {
	...
	/**
	 * 
	 * Climb the Leaderboard using Brute force O(n^2)
	 * 
	 * @param scores
	 * @param alice
	 * 
	 ...
	 */
	public static int[] climbingLeaderboardLowest(int[] scores, int[] alice) {

		LinkedHashSet<Integer> _scores = new LinkedHashSet<Integer>();
		for (int temp : scores)
			_scores.add(temp);

		int alice_rank = 1, alice_ranks[] = new int[alice.length];
		for (int i = 0; i < alice.length; i++) {
			for (Integer score : _scores) {
				if (alice[i] < score)
					alice_rank++;
				else if (alice[i] >= score)
					break;
			}
			alice_ranks[i] = alice_rank;
			alice_rank = 1;
		}
	...
	/**
	 * Climbing  the Leaderboard using a fast way O(n)
	 * 
	 * */
	public static int[] climbingLeaderboardFastest(int[] scores, int[] alice) {

		List<Integer> _scores = new ArrayList<Integer>();
		int prev = -1;
		...
		while (i < _scores.size() && j>=0){
			if (alice[j] >= _scores.get(i)) {
				j--;
				rank[j+1] = current;
			}
			else {
		...
		// loop for alice's score that left over
		while(j>=0) {
			rank[j] = current;
			j--;
		}
	...
	public static void main(String[] args) {
		...
		Util.printArray(climbingLeaderboardFastest(scores, alice));
	}
}
